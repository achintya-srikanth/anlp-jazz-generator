...
class TransformerXL(object):
    ...
    # initialize
    ########################################
    def __init__(self, event2word, word2event, checkpoint=None, is_training=False, training_seqs=None, rvq_quantizers=8, codebook_size=256):
        # load dictionary
        ...
        self.codebooks = [
            tf.compat.v1.get_variable(
                f'codebook_{i}',
                shape=[codebook_size, self.d_model],
                initializer=tf.compat.v1.keras.initializers.he_normal()
            ) for i in range(rvq_quantizers)
        ]
        # load model
        self.load_model()
    
    #######################################
    # TensorFlow implementation of RVQ
    #######################################
    def residual_quantize(emb, codebooks):
        residuals = emb
        quantized = 0.0
        for cb in codebooks:
            # Compute distances
            dists = tf.norm(
                tf.expand_dims(residuals, 2) - tf.expand_dims(cb, 0), 
                axis=-1
            )
            # Find nearest indices
            indices = tf.cast(tf.argmin(dists, axis=-1), tf.int32)            # Gather quantized vectors
            q_vectors = tf.gather(cb, indices)
            quantized += q_vectors
            residuals -= q_vectors
        return quantized
    ...
    def load_model(self):
        ...
        with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope()):
            ...
            # RVQ codebooks (8 codebooks x 256 entries)
            self.codebooks = [
                tf.compat.v1.get_variable(
                    f"codebook_{i}",
                    [256, self.d_model],
            ...
            # --- RVQ Quantization Start ---
            # Embedding lookup
            emb = tf.nn.embedding_lookup(self.embedding, xx)  # [seq_len, batch_size, d_model]
            
            # Residual quantization
            quant_emb = tf.zeros_like(emb)
            residual = emb
            for cb in self.codebooks:
                # Calculate distances [seq_len, batch_size, codebook_size]
                dists = tf.norm(
                    tf.expand_dims(residual, 2) - 
                    tf.expand_dims(cb, 0), 
                    axis=-1
                )
                # Find nearest indices [seq_len, batch_size]
                indices = tf.argmin(dists, axis=-1)
                # Gather quantized vectors
                q_vectors = tf.gather(cb, indices)
                quant_emb += q_vectors
                residual -= q_vectors
            # --- RVQ Quantization End ---
            ...
            loss, self.logits, self.new_mem = modules_rvq.transformer(
                ...
                untie_r=False,
                proj_same_dim=True,
                skip_embed=True  # NEW FLAG TO DISABLE INTERNAL EMBEDDING
                )
        
        self.commit_loss = tf.reduce_mean(tf.square(quant_emb - emb))
        self.avg_loss = tf.reduce_mean(loss) + 0.25 * self.commit_loss # 0.25 from EnCodec paper
        # vars
        ...
        for e in range(151, 300):
            ...
            for i in range(num_batches):
                ...
                for j in range(self.group_size):
                    ...
                    print('>>> Epoch: {}, Step: {}, Loss: {:.5f}, Time: {:.2f}'.format(e, gs_, loss_, time.time()-st))   #GT: Comment out
            
            print ('[epoch {} avg loss] {:.5f}'.format(e, np.mean(total_loss)))
            if e >= 0:
                
                if e % 15 == 0: #or 0.085 <= np.mean(total_loss) <= 0.10 or np.mean(total_loss) <= 0.8:

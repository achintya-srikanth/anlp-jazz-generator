...
def transformer(dec_inp, target, mems, n_token, n_layer, d_model, d_embed,
                ...
                scope='transformer',skip_embed=True):
    """
    cutoffs: a list of python int. Cutoffs for adaptive softmax.
    tie_projs: a list of python bools. Whether to tie the projections.
    perms: a list of tensors. Each tensor should of size [len, bsz, bin_size].
        Only used in the adaptive setting.
    skip_embed: skip embedding in transformer block if true (already quantized with rvq)
    """
    new_mems = []
    with tf.compat.v1.variable_scope(scope):
        if untie_r:
        ...
        if not skip_embed:  # ONLY EMBED IF NOT PRE-QUANTIZED
            ...
            with tf.compat.v1.variable_scope('embedding'):
                lookup_table = tf.compat.v1.get_variable('lookup_table', [n_token, d_model], initializer=initializer)
                proj_W = None  # No projection needed for RVQ embeddings
                shared_params = [lookup_table, proj_W]
                embeddings = dec_inp # Use pre-quantized
